...
FROM cats 
WHERE name = "Murzik") AS info_murzik
ON s.id = info_murzik.shops_id;
  
SELECT shops.*, cats.name
FROM shops, cats
WHERE  cats.id = shops.id AND cats.name = "Murzik"

--3.Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT shops.*, info.* 
FROM shops  
JOIN (SELECT name, shops_id  FROM cats
WHERE name NOT IN ("Murzik", "Zuza")) AS info
ON shops.id = info.shops_id;

-- 4.Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.

SET @date = (DATE '2020-02-05');

SELECT analysis.an_name, analysis.an_cost, orders.ord_datetime
FROM Analysis
JOIN orders
ON analysis.an_id = orders.ord_an AND DATE(orders.ord_datetime) BETWEEN @date AND DATE_ADD(@date, interval 1 WEEK);
